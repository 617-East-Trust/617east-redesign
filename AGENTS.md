...
| **Production host** | IONOS VPS `40.160.233.147` (`617east`) | Live `https://617east.com` (IONOS = domain + VPS) |
...
- Wave 6 (2026-08-02): service→location internal links; blogs NC sales-tax/nexus + veteran-owned formation; sitemap now **38** URLs (includes `/opportunity-review`); citations kit `Projects/Infrastructure/citations-617east.md`; GBP automations recreated (weekly post + daily review replies).
- Wave 7 (2026-08-04): first-party analytics pipeline — same-origin collect proxy; dual-write consent events; fingerprint opt-in off by default. Ops: `WAVE7-PIPELINE-OPS.md`.
- **Pipeline LIVE on VPS (2026-08-04):** `/opt/analytics-pipeline`; `ANALYTICS_COLLECT_URL=http://analytics-vector:8080` via Docker net `analytics-pipeline_analytics-net`; proxy sends NDJSON. Verified collect **204** + ClickHouse rows. Handoff: `Records/617east_analytics_pipeline_deploy_20260804_053600.md`.
...
- **Pipeline env (VPS):** `ANALYTICS_COLLECT_URL` + `ANALYTICS_COLLECT_TOKEN` (Bearer; server-only). Unset → pipeline off; GTM/Clarity still consent-gated.
...
```bash
ssh root@40.160.233.147 'caddy reload --config /etc/caddy/Caddyfile'
```
Host Caddy (`/etc/caddy/Caddyfile`) owns `:80`/`:443`: apex, www, discover, n8n. `n8n-caddy-1` is parked (cannot bind those ports).
